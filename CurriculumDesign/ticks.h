...
#define portMAX_INTERRUPTS				( ( uint32_t ) sizeof( uint32_t ) * 8UL )
#define portNO_CRITICAL_NESTING 		( ( uint32_t ) 0 )
#define portTICK_PERIOD_MS			( ( uint32_t ) 1000 / 100 )
#define portINTERRUPT_TICK				( 1UL )

#define portSOAK_TEST
#ifndef portSOAK_TEST
#define portDELETE_SELF_THREAD_PRIORITY			 THREAD_PRIORITY_HIGHEST
...
#else
#define portDELETE_SELF_THREAD_PRIORITY			 THREAD_PRIORITY_TIME_CRITICAL 
...
static DWORD WINAPI prvSimulatedPeripheralTimer(LPVOID lpParameter)
{
	...
	{
		xMinimumWindowsBlockTime = (unsigned int)xTimeCaps.wPeriodMin;
	...
	{
		xMinimumWindowsBlockTime = (unsigned int)20;
	}
...
size_t *pxPortInitialiseStack(size_t *pxTopOfStack, ProcessFunction_t pxCode, void *pvParameters)
{
...
static uint32_t prvProcessTickInterrupt(void)
{
	...
	ulSwitchRequired =(uint32_t)increaseTicks();
...
void vPortCloseRunningThread(void *pvTaskToDelete, volatile uint32_t *pxPendYield)
{
	xThreadState *pxThreadState;
	void *pvThread;
	uint32_t ulErrorCode;
